...
визуализацию карты маршрутов в виде SVG-изображения. Пора вернуться к
выводу JSON. В этом задании нужно добавить обработку нового типа
запросов к транспортному справочнику — запрос на построение карты.
...
1. описание маршрута «114», который соединяет остановки «Морской вокзал» и «‎Ривьерский мост»;
2. настройки визуализации карты;
3. запросы на визуализацию и получение информации о маршруте «114» и остановке «‎Ривьерский мост».

🔗 [input.json](https://code.s3.yandex.net/CPP/input2.json)

Ожидаемый вывод:

🔗 [output.json](https://code.s3.yandex.net/CPP/output.json)
...
🔗 [s10_final_opentest](https://code.s3.yandex.net/CPP/s10_final_opentest_.zip)

### Как будет тестироваться ваш код

Программе
из stdin поступают валидные JSON-документы с описанием маршрутов и
запросов к справочнику. Данные в stdout будут сравниваться с ожидаемым
значением. Вывод вещественных чисел должен осуществляться стандартно,
без модификаторов вывода.

### Доп инфо

В ответ на запрос `"Map"`
, выведите SVG-изображение карты в поток `std::ostringstream`
. Затем извлеките строку из этого потока и сохраните в качестве ключа `"map"`